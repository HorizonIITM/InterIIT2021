...
const updatedJsonData = jsonData.map(point => {
  ...
  const updatedType = point.Type.replace('(', '').replace(')', '').replace('?', '').split(',').map(char => {
    switch (char) {
      case 'P':
  ...
  const updatedCat = point.Cat.replace('(', '').replace(')', '').replace('?', '').split(',').map(char => {
    switch (char) {
      case 'A':
  ...
  const updatedPos = point.Pos.replace('(', '').replace(')', '').replace('?', '').split(',').map(char => {
    switch (char) {
      case 'o':
        return 'optical'
      case 'x':
